...
- **매니저(Manager) 클래스 네트워크 적용**
  ...
  - Barrel 설치물이 의도대로 동작하지 않아 디버깅 과정에서 많은 시행착오를 겪음
  - Rigidbody에 가해지는 Addforce의 동기화가 이루어지지 않아 각자 다른 방향으로 날아가도록 보이는 버그가 있었는데
    이것은 마스터에서 콜백으로 RPC를 실행시켜 동일하게 넉백 수치를 적용하고 클라이언트에는 이펙트만 적용하는 방식으로 해결하였음
